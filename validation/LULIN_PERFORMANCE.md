...
The same atlas, read for what limits each rig rather than for headline numbers:
...
the underlying noise model was validated end-to-end at obs/pred 1.011
(`ENDTOEND_RESIDUALS.md`), so this one abscissa is anchored to data and the
rest is model extrapolation. Regenerate with
